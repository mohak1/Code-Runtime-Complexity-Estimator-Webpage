...
//adding stuff for google vis
google.load("visualization", "1", { packages: ["corechart"] });
//google.setOnLoadCallback(drawChart);
//<-do this stuff inside setOnLocalCallback->
...
//make the textarea skeletal funciton change as the language selection is changed
document.getElementById("prog_language").onchange = function(){
    if (document.getElementById("prog_language").value=="C"){
        //write C skeletal code
        //document.getElementById("code_textbox").innerHTML="C Code";
        ...
        //write C++ skeletal code
        //document.getElementById("code_textbox").innerHTML="C++14 Code";
        ...
        //write C++ skeletal code
        //document.getElementById("code_textbox").innerHTML="C++17 Code";
        var code_skeletal = "#include <iostream>\n"+
        "using namespace std;\n\n"+
        "int main() {\n"+
        "    // your code goes here\n"+
        ...
        //write GO skeletal code
        //document.getElementById("code_textbox").innerHTML="GO Code";
...
//adding buttons for the input cases
var num_buttons = 4;
function add_new_input_case(val=""){
    //code to add more text fields
    num_buttons++;
    var form_name = document.getElementById("form");
    //for giving the textbox a nubmer and nextline char
    var div_element = document.createElement("div");
    newline_num = document.createTextNode(num_buttons+".");
    // form_name.appendChild(document.createElement());
    ...
    if (val.length>0){
        //this isn't the default value, add this value to the text box
...
document.getElementById("try_fibonacci").onclick = function(){
    ...
    //adding fibonacci code to the textarea
    document.getElementById("code_textbox").value = fibonacci_code;

    //chagne the language to python3
    document.getElementById("prog_language").value = "Python 3.x"

    //add the test textboxes and fill value in them
...
document.getElementById("try_bubblesort").onclick = function(){
    ...
    //adding bubblesort code to the textarea
...
document.getElementById("submit_button").onclick = function(){
    ...
    //check if the code text area is empty
    var text_area_content = document.getElementById("code_textbox").value;
    if(text_area_content.length < 1){
        //there is no code in the text area
        alert("It's impossible to estimate complexity when there "+ 
        "isn't any code to begin with. Please type some code in the box and try again.")
    }else{
        //code is alright, set the flag to 1
        code = 1;
    }

    //check how many input cases have been added
    //send an error if these are less than 4
    var non_empty=0;
    for(var i=1; i<=num_buttons; i++){
        //check if at least 4 of these are non empty
    ...
    if(non_empty<4){
        ...
        //inputs are alright, set the flag to 1
        inputs = 1;
    }

    //collect all the non empty test case fields
    var tests = [];
    var testString = "";
    if(code==1 && inputs==1){
        //disable submit button
        document.getElementById("submit_button").disabled = true;
        //the inputs are valid, send to the api
        for(var i=1; i<=num_buttons; i++){
            //send this input to the api if it isn't empty
            var input_content = document.getElementById("test"+i).value;
            if(input_content.length>0){
                //make a string instead of an array
                testString = testString + input_content + "EnDoFtEsTcAsE";
                //tests.push(input_content);
            }
        }
        ...
        //get the code language from the selection dropdown
        var lang = document.getElementById("prog_language").value;
                
        //encoding the code as a string
        var code = encodeURIComponent(user_code);
        var code = code.replaceAll("%2F", "%SLASH");
        
        //encoding tests as a string
        //var tests = encodeURIComponent(tests);
        var testString = encodeURIComponent(testString);
        //var tests = tests.replaceAll("%2F", "%SLASH");
        var testString = testString.replaceAll("%2F", "%SLASH");
        
        //send the code and the tests to the api
        var req = new XMLHttpRequest();
        var base_url = "https://https://mohak-code-complexity-api.herokuapp.com/"
        var endpoint = "complexity";
        
        // var url = base_url+endpoint+"/"+code+"/"+tests+"/"+lang;
        var url = base_url+endpoint+"/"+code+"/"+testString+"/"+lang;
       
        req.open("GET", url, true);
        req.send()
        // alert("Sending the code and test cases to the API.\nPlease wait...")
